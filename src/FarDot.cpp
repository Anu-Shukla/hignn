...
void HignnModel::FarDot(DeviceDoubleMatrix u, DeviceDoubleMatrix f, DeviceDoubleMatrix divM) {
  ...
  DeviceDoubleMatrix cDivMMiddlePool("cDivMMiddlePool",
...

    {
      ...
      Kokkos::parallel_for(
          Kokkos::RangePolicy<Kokkos::DefaultHostExecutionSpace>(
              0, totalCoord * 3),
          [&](const int i) { hostCGradPairs(i / 3, i % 3) = 0.0; });
      Kokkos::fence();
      ...
      for (int k = 0; k < 9; k++) {
        ...
        for (int i = 0; i < totalCoord; i++) {
          ...
          const float gradValue = gradPtr[3 * i + row];
          if (std::isfinite(gradValue)) {
            hostCGradPairs(i, col) += gradValue;
          }
        }
      }
            ...
            KOKKOS_LAMBDA(
              ...
              Kokkos::parallel_for(
                  ...
                  [&](const int i) {
                    ...
                    Kokkos::atomic_add(&divM(indexJStart + index, row),
                                       -divMSum);
                  });
            });
        Kokkos::fence();
